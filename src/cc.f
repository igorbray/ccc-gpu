...
c$$$      PARAMETER(JMAX=50)
      ...
      real*8 dx,deta,xlmin,dfc(0:jmax),dgc(0:jmax),dfcp(0:jmax),
     >   dgcp(0:jmax)
...
C
     ...
     X     FPMAX,FPMIN,FPLMAX,FPLMIN / 1D+307,1D-307 ,706.89363d0,
     >   -706.89363d0/,
c$$$     X     FPMAX,FPMIN,FPLMAX,FPLMIN / 1D+290,1D-290 ,667.749676968D+0,
c$$$     >   -667.749676968D+0 /,
c$$$     X     FPMAX,FPMIN,FPLMAX,FPLMIN / 1D+60,1D-60 ,140D+0, -140D+0 /,
     X     R20,ASYM,XNEAR,NDROP / 3., 3., .5, 5 /,
     X     ACCUR, ACC8, ACC16 / 1D-10, 3D-16, 2D-16 /
!!!     X     ACCUR, ACC8, ACC16 / 1D-10, 2.3D-16, 2D-16 /
c$$$     X     ACCUR, ACC8, ACC16 / 1D-13, 2D-16, 2D-16 /
...
!      print*, 'in cc ACC8=',ACC8
      itmp = logam(ACC8)
!      CALL LOGAM(ACC8)
!      print*, '2 in cc ACC8=',ACC8
!      itmp = logam(ACC8)
      ACCH  = SQRT(ACCUR)
      ACCB  = SQRT(ACCH)
      RERR = ACCT
...
C  Igor replaced the commented out statement as FCL gets too small
      IF(real(fcm)*real(fcl)+aimag(fcl)*aimag(fcm).LT.ZERO) FCM=-FCM
c$$$      IF(REAL(FCM/FCL).LT.ZERO) FCM  = - FCM
      GAM = (F - P)/Q
         TA = ABSC(GAM + PM)
...
C
      ...
      W = FCM / FCL
         ...
         IF(MODE.GE.3) GO TO 240
            IF(ABSC(F-PQ1) .LT. ACCH*ABSC(F) .AND. PR) then
               print*,'F,PQ1,ACCH,F,PR:',F,PQ1,ACCH,F,PR 
...
c$$$         IF(RERR.GT.ACCB) WRITE(6,1070) RERR
         IF(RERR.GT.accb) IFAIL = -4
c$$$         IF(RERR.GT.0.1) IFAIL = -4
c$$$         if (dx.gt.1d-7) then
c$$$         if (abs((dfc(lrange)-real(fc(1)))/(1d-10+dfc(lrange))).gt.1e-3)
c$$$     >      print*,'xx,dfc,fc:',real(xx),dfc(lrange),real(fc(1))
c$$$         if (abs((dgc(lrange)-real(gc(1)))/(1d-10+dgc(lrange))).gt.1e-3)
c$$$     >      print*,'xx,dgc,,gc:',real(xx),
c$$$     >      dgc(lrange),real(gc(1))
c$$$         endif 
         RETURN
C
...
c$$$      REAL*16 AR,BR,GR,GI,DR,DI,TR,TI,UR,UI,FI,FI1,DEN
...
!      print*,'N,SC2,TC2:',N,SC2,TC2
      if (abs(sc2).gt.sqrt(fpmax).or.abs(tc2).gt.sqrt(fpmax)) goto 40 !Trying to stop overflows, Igor
      SL=SL+SL2
      TL=TL+TL2
      SC=SC+SC2
      TC=TC+TC2
      ...
      TC1=TC2
      FCL  =  TL + SL*TANL
      IF(ABS(FCL).GT.FPMAX .OR. ABS(FCL).LT.1./FPMAX) GO TO 40
c$$$  IF(ABSC(FCL).GT.FPMAX .OR. ABSC(FCL).LT.1./FPMAX) GO TO 40
      GSUM = (TC + SC*TANL) / FCL
      G(N) = GSUM - GLAST
      GLAST = GSUM
         AT = ABSC(G(N))
