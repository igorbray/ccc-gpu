...
C
      ...
      COMPLEX*16 Z,U,V,H,R,CDIGAM,CLOGAM,SER, LOGAM 
      DIMENSION B(15),BN(15),BD(15)
      real*8::ACCUR,  HL2P, ACC
      integer::NT=0
C
     ...
     X     BN(15),BD(15)  / 8615841276005D+0, 14322D+0/
      DATA FPLMIN / -667.749676968D+0 /
c$$$      DATA FPLMIN / -140D+0 /
C
      ...
      H=H*V
   10 A=A+ATAN2(D,C)
C  Had problems with overflows here, Igor.
 20   temp = abs(h) ** 2

      H=DCMPLX(HALF*LOG(temp),A)
c$$$   20 H=DCMPLX(HALF*LOG(REAL(H)**2+IMAG(H)**2),A)
      V=V+ONE
   30 R=ONE/V**2
      SER = B(NT)
      DO 40 J=2,NT
...
C
  110 WRITE(LERR,1000) 'CDIGAM:',X
      print*,'CDIGAM: U, X, A, ACCUR',u,x,a,accur
      CDIGAM=ZERO
      RETURN
C
      ENTRY LOGAM(ACC)
!      print*, 'in logam ACC=', ACC
C
